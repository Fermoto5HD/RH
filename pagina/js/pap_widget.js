...
RecorridosToDestinoMap.prototype.geocodeDestinos=function(destino){
    ...
    jQuery("body").bind('all-geocoded',function(e){
    	//console.log('all-geocoded', destino, cp.initiating)
        if(!cp.initiating){
            cp.initiating=true;
	...
	jQuery('.open.search').click(function(e){
		if(!jQuery('#'+cp.id).hasClass('expanded')){
		    if(cp.extended_height_inc==null){
				...
				var t="#"+cp.id+" .recorridos{height:"+map_height+"px;}";
...
RecorridosToDestinoMap.prototype.initAutoComplete=function(){
	...
	<div class='origen search autocomplete'> \
		<form accept-charset='iso-8859-1'> \
			<label for='search-value'>¿Desde donde salís?</label> \
	...
	<div class='destino search' style='display: none;'> \
		<form accept-charset='iso-8859-1'> \
...
RecorridosToDestinoMap.prototype.zoomToMarkers=function(){
	...
	var ms=this.map.getMarkers();
	this.bounds = new OpenLayers.Bounds();
	var self = this;
	//console.log(this.map.getMarkers());

	if(typeof ms!="undefined"){
		jQuery.each(ms, function(k, v) {
			console.log(self)
			self.bounds.extend(v.lonlat);
//			console.log(v)
		});
		/*
